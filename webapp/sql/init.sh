...
mysql -u${ISU_DB_USER} -h${ISU_DB_HOST} -P${ISU_DB_PORT} -p${ISU_DB_PASSWORD} < "${DB_DIR}/isucoin.sql"
gzip -dc "$DB_DIR/initializedata.sql.gz" | mysql -u${ISU_DB_USER} -h${ISU_DB_HOST} -P${ISU_DB_PORT} -p${ISU_DB_PASSWORD}

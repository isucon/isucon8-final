...
`POST|DELETE` による注文(または取り消し)は即座に `GET` による注文履歴に反映されなければならない。  
ただし、注文と同時に銀行への決済予約の失敗によって自動的にキャンセルとなった場合は、注文直後であっても注文履歴に返却されないケースも発生する可能性がある。  
...
ただし、traded_ordersにはリクエストしたcursorをレスポンスで返すcursorそれぞれの間の取引は含まれていなければならない。 

...
以下のいすこん銀行アカウントをベンチマークに影響しない富豪アカウントとして自由に使って構いません。
それぞれ残高は10億あります。
...
アプリケーションにはweb/ios/androidなど複数のクライアントが存在しています。
そのためswaggerによるAPIドキュメントがあります

docs/swagger/webapp.yml
...
※ TBD APIドキュメントと同様に別仕様書にしたほうが良いか?

### 外部API

アプリケーションはhttpプロトコルを用いた2つの外部APIを利用しており、ビジネス的な観点でそれらを必ず利用しなければなりません

#### いすこん銀行API (isubank)

口座の入出金や残高確認を行うAPI

詳細仕様 docs/isubank-API.md
...
ログの欠落は認められず、また、遅延は10秒以内としなければならない

詳細仕様 docs/logger-API.md
